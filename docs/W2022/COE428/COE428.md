...
For this example, we say the running time is $T(n) = 3$,
...
1.  In line 1, its indexing an array, $s[0]$, then
    assigning it to $x$
...
2.  In line 2, inside the for loop:
    ...
    -   On each iteration, it makes a comparison, $<$, for
        $n$ times

3.  In line 3, the if statement repeats for :

    -   Its indexing an array, $s[i]$, then makes a
        comparison, $>$

    -   In line 4, there's two possibilities that could occur:

        -   If true, then its indexing an array, $s[i]$,
            then assigning it to $x$

        -   If false, then it won't execute

    -   At the end, $i++$, is incremented, then assigned
        back to $i$

4.  The last line returns $x$

The number of primitive operations executed by algorithm can be
characterized in two ways:

-   Best-Case Analysis:
    $T(n) = 2 + 1 + n + (n-1)(2 + 0 + 2) + 1 = 5n$

-   Worst-Case Analysis:
    $T(n) = 2 + 1 + n + (n-1)(2 + 2 + 2) + 1 = 7n - 2$
...
-   Arithmetic series:
    $\displaystyle\sum_{k = 0}^{n} k = 1 + 2 + \cdots + n = \frac{n(n+1)}{2}$

-   Geometric series:
    $\displaystyle\sum_{k = 0}^{n} x^k = 1 + x + x^2 + \cdots + x^n = \frac{1-x^{n+1}}{1-x}$

\... and the following terms which you might recall from coding:

-   $\lfloor x \rfloor$ or `floor(x)` is largest integer
    less than to equal to $x$.

-   $\lceil x \rceil$ or`ceiling(x)` is least integer
...
:   A permutation (reordering)
    $\langle a'_1, a'_2, \dots, a'_n \rangle$ of the input
    sequence such that
...
where $t_j$ is the number of times the while loop is
...
$$\sum_{j=2}^n t_j \to \sum_{j=2}^n 1 = (n - 2) + 1 = n - 1$$
Substituting this in the equation to the running time simplifies
$T(n)$ to $${
    \begin{split}
         T(n) &= c_1n + c_2(n - 1) + c_4(n - 1) + c_5(n - 1) + c_8(n - 1) \\
             &= (c_1 + c_2 + c_4 + c_5 + c_8)n - (c_2 + c_4 + 5 + c_8)
    \end{split}}$$ or equivalently, if we let $c_n$
simplify to some constants $a$ and $b$ then
$$T(n) = an - b$$
...
In this example, $t_j$ has to compare with all elements to
the left $j$-th positioncompare with $j - 1$
elements.

Thus, $t_j = j$, we can derive the number of comparisons
for every outer loop iteration
$$\sum_{j = 2}^{n} t_j \to \sum_{j = 2}^{n} j = 2 + 3 + 4 + \dots + n = \bigg[\sum_{j=1}^n j\bigg] - 1 = \frac{n(n+1)}{2} - 1$$
and as well the number of moves inside the while loop:
$$\sum_{j = 2}^{n} (t_j - 1) \to \sum_{j = 2}^{n} (j - 1) = 1 + 2 + 3 + \dots + n - 1 = \frac{n(n-1)}{2}$$
Substituting this in the equation to the running time simplifies
$T(n)$ to $${
    \begin{split}
    ...
    \end{split}}$$ or equivalently, if we let $c_n$
simplify to some constants $a$, $b$ and
$c$ then $$T(n) = an^2 + bn - c$$
...
:   Calls [`Merge-Sort(A,p,q)`]{style="background-color: light-gray"} to
    split the left children with different values of $q$
    and $r$ (passing parameter by value).
...
-   The time to split deck takes can be denoted by $c_1$,
    as it takes constant timedoes not depend on any input.

-   The time to sort left pile and sort right pile can be denoted by
    $2T(n/2)$, due to recursion, where the size is now
    divided by two, $n/2$.

-   The time to merge piles can be denoted by $c_2n + c_3$,
...
$$T(n) = c_1 + T(n/2) + T(n/2) + c_2n + c_3$$ Our goal is
to determine the most rapidly growing term in $T(n)$ and so
we can set a few rules. We set constants $c_n$ to either:

-   $0$, if they will not be significant in the most
    rapidly growing term or ...

-   $1$, if they will be

For $T(n)$, when $n > 1$, we can set
$c_1$ and $c_3$ to $0$ and
$c_2$ to $1$, which simplifies to:
$$T(n) = \begin{cases}c_1 & \text{if}\ n = 1 \\ 2T(n/2) + n & \text{if}\ n > 1 \end{cases}$$
...
In order to solve the recurrence, when $n > 1$, we need a
base case, so for simplicity, $T(1) = 0$ and we can make a
deduction from it.
...
by a function of $n\log_2{n}$, which can also be deducted
by drawing a recursion tree.

1.  We start by representing $T(n) = 2T(n/2) + n$ as a
    graph where we put the non-recursive part ($n$ in this
...
$n$. Let's denote the depth or height of the tree as
$h$ and so we can say the time complexity is
:::

$$T(n) = n \times h$$ It will eventually reach the base
case which we set to some constant when $n=1$, where
$T(1) = 1$. We can rewrite the fraction in terms of the
depth, $h$, where
$$\frac{n}{2^h} = 1 \to n = 2^h \to h = \log_2{n}$$ Thus,
the time complexity is $$T(n) = n\log_2{n}$$ If you recall
the order-of-growth from Lecture 2, we know that
$n\log{n} < n^2$, and so merge sorting beats insertion sort
...
selection sort:
$$T(n) = c_1n + c_2(n-1) + c_3\sum_{j=2}^n j + c_4\sum_{j = 2}^n (j - 1) + c_5(n-1)$$
As we have previously done with insertion sort, we can simplify the
summation using the arithmetic series $${
    \begin{split}
    ...
    \end{split}}$$ or equivalently, if we let $c_n$
simplify to some constants $a$, $b$ and
$c$ then $$T(n) = an^2 + bn - c$$ Comparing it
to the other two algorithms discussed, selection sort is on par with
insertion sort in the worst-case scenario and so merge sorting is better than selection sort as well.

# Complexity Analysis

## Asymptotic Notations 
As covered briefly in the
...
-   Big-Oh notation, $\text{O}()$, for the upper bound or
    worst-case complexity

-   Big-Omega notation, $\Omega()$, for the lower bound or
    best-case complexity

-   Theta notation, $\Theta()$, for the average bound or
...
different sorting algorithms with varying time complexity:
  ...
  ---------------- --------------------------------- -----------------------------------
     Algorithm              Time Complexity          
                                 Best                               Worst
   Insertion Sort      $\Omega(n)$         $\text{O}(n^2)$
     Merge Sort     $\Omega(n\log{n})$   $\text{O}(n\log{n})$
   Selection Sort     $\Omega(n^2)$        $\text{O}(n^2)$
  ---------------- --------------------------------- -----------------------------------
...
1.  Ignore the constants: $$5n \to n$$

2.  Certain terms dominate other, which we ignore lower order terms:
    $$\text{O}(1) < \text{O}(\log{n}) < \text{O}(n) < \text{O}(n\log{n}) < \text{O}(n^2) < \cdots < \text{O}(2^n) < \cdots < \text{O}(n!) < \text{O}(n^n)$$

It might be easier to understand if we have examples to determine the
big-Oh notation.

::: exampleT
**Example 3.1**. *Determine the upper bound $\text{O}()$
for $f(n)$:*
...
-   *$f_C(n) = 5 + (15 \cdot 20)$ is
    $\text{O}(1)$*

-   *$f_D(n) = n^2\log{n} + n$ is
    $\text{O}(n^2\log{n})$*
:::
...
the running time. While the function $\text{O}(n^2)$ is
true for $f_B(n)$, the function $\text{O}(n)$
is the closest to $f_B(n)$.
:::

The rules for determining the $\text{O}()$ complexity are
as listed:

1.  If $g(n) = \text{O}(G(n))$ and
    $f(n) = \text{O}(F(n))$, then:
    $$f(n) + g(n) = \text{O}(F(n)) + \text{O}(G(n)) = \text{O}(\text{max}[F(n), G(n)])$$
    $$f(n) \cdot g(n) = \text{O}(F(n)) \cdot \text{O}(G(n)) = \text{O}(F(n) \cdot G(n))$$

2.  If $g(n) = \text{O}(kG(n))$, where $k$ is
    a constant, then $g(n) = \text{O}(G(n))$.

3.  If $f(n)$ is a polynomial of degree
    $d\ (P(n) = \sum_{i=0}^d a_in^i$ where
    $a_d \neq 0)$, then $f(n)$ is
    $\text{O}(n^d)$.
...
The rules for determining $\text{O}()$ complexity is also
true for determining the $\Omega()$ complexity. Let's use
an example from before.

::: exampleT
**Example 3.2**. *Determine the lower bound $\Omega()$ for
$f(n) = 2n + 3$:*

1.  *If we look at the order-of-growth for functions,
    $T(n)$ belongs to the linear function, $n$
    and if we define our lower and upper bounds as such
    $${\overunderbraces{&\br{2}{\text{Lower bound}}}%
        {&1 < \log{n} < \sqrt{n} <& n &< n\log{n} < n^2 < n^3 < \cdots < 2^n < 3^n < \cdots < n! < n^n&}
        {& &\br{2}{\text{Upper bound}}}}$$*

2.  *So the lower bound can be defined by any of the following:
    $$\Omega(1) < \Omega(\log{n}) < \Omega(n)$$*

3.  *Similar to the upper bound, we want the function closest to
    $f(n)$ and so $f(n) = 2n + 3$ is
    $\Omega(n)$.*
:::
...
Equivalently, $f(n)$ is $\Theta(g(n))$ if and
only if $f(n)$ is both $\text{O}(g(n))$ and
$\Omega(g(n))$.

One notable example which we used previously is the function
$f(n) = 2n + 3$, which as we demonstrated in previous
notations are $\text{O}(n)$ and $\Omega(n)$,
thus $f(n)$ is $\Theta(n)$.

## Complexity of Code Structures 

Loops are considered as dynamic if they depend on input size, otherwise they are
static statements, everything within a loop is considered as static
statementtakes a constant amount of time, $\text{O}(1)$.
The complexity is determined by:

-   number of iterations in the loops $\times$ number of
    static statement
...
You can see that the loop executes $3$ times or in general,
we can say $n$ times. Thus, the time complexity is:
$$n \cdot 1 = \text{O}(n)$$
...
In both examples, the loop executes for $n$ times. When we
...
complexity is:
$$\underbrace{n \cdot 1}_{\substack{\text{The first} \\ \text{for loop}}} + \underbrace{n \cdot 1}_{\substack{\text{The second} \\ \text{for loop}}} = 2n = \text{O}(n)$$
...
Let's list out each iterations of the loop, till $k$
...
complexity is:
$$(\log_2{n} + 1) \cdot 1 = \text{O}(\log_2{n})$$
...
As we covered earlier, the following for loop executes $n$
times. Suppose now we have an inner loop, which also executes
$n$ times, then the statement is run
$n \times n$ times. Thus, the time complexity is:
$$(n \cdot n) \cdot 1 = \text{O}(n^2)$$
...
$$\text{O}(if-else) = \text{O}\Big(\text{max}\Big[\text{O}(\text{condition1}), \text{O}(\text{condition2}), \dots, \text{O}(\text{branch}1), \text{O}(\text{branch2}), \dots\Big]\Big)$$
...
complexity is: $$1 + n = \text{O}(n)$$ or equivalently
$$\text{O}\Big(\text{max}\Big[\text{O}(1), \text{O}(1), \text{O}(n)\Big]\Big) = \text{O}(n)$$
...
worst-case running time $T(n)$ of merge-sort procedure by
the recurrence:
...
:   Guess the recurrence formula after $k$ iterations (in
    terms of $k$ and $n$).
...
:   Put $k$ back into the formula to find a potential
    closed form.
...
1.  The easiest way to find a pattern, is by simply writing out the
    first few iterations. Let's denote $k$ as our number of
...
2.  Our goal is to generalize this for $k$ iterations. In
    other words, relating each of the constants to $k$. We
    can rewrite it as such $${\begin{split}
...
3.  We know the base case is set to $T(1) = 1$. From our
    general formula, we can determine how many iterations there are in
    terms of $n$ to reach the base case, by solving for
    $k$. $${\begin{split}
                \frac{n}{2^k} = 1 \ \to\ n &= 2^k \\
                k &= \log_2{n}
            \end{split}}$$

4.  Substituting $k = \log_2{n}$ back into the general
    formula, we get a potential closed form, as $T()$ is no
    longer inside our formula. $${\begin{split}
                T(n) &= 2^{\log_2{n}} \cdot T(n/2^{\log_2{n}}) + n\log_2{n} \\
                &= n + n\log_2{n}
            \end{split}}$$

5.  We can guess that the solution is
    $T(n) = \text{O}(n\log{n})$. However, we need a
    definite proof that this is true, by using mathematical induction
    for the following statement.
    $$0 \leq T(n) \leq cn\log{n} \hspace{1cm} \exists c > 0,\ \forall n \geq n_0$$
...
can choose the constant $c$ large enough so that
$T(n) \leq cn\log{n}$ is true.

1.  Remember, the base case is $T(1) = 1$. Then for
    $n = 1$, it yields
...
2.  For asymptotic notation we can specify a specific bound,
    $\forall n \geq n_0$, where $n_0$ is
    something we can choose. Thus $n_0 = 2$, removing it
...
Let's assume $n$ is some power of $2$ or
$n = 2^k$, for sake of convenience.

Base Case:

:   Let $k = 1$ or $n = 2$ then:
    $$T(2) = 2T(1) + 2 = 2 + 2 = 4 \leq c(2)\log{2}$$ We
...
:   We will now assume that our proposition,
    $T(n) = \text{O}(n\log{n})$, holds true for
    $k -1$, which equivalently is $n/2$,
    therefore: $$T(n/2) \leq c(n/2)\log{(n/2)}$$ To prove
    the inductive step, one assumes the induction hypothesis for
    $k-1$ and then uses this assumption to prove that the
    statement holds for $k$. If instead, we assume our
    hypothesis to hold for $k$, then we must prove it holds
    for $k+1$.
...
:   From our hypothesis, prove the guess of correct for
    ...
    $$T(n) = 2T(n/2) + n$$ Since we know
    $T(n/2) \leq c(n/2)\log{(n/2)}$, then we can rewrite it
    as such: $${\begin{split}
...
1.  The non-recursive part, $n$, will be the parent node
    and the recursive part, $T(n-1)$, will be the child
...
2.  Expand on $T(n-1)$, similar to the previous step.

    ::: center
    ![image](Figure/4/ex_recursion2.png){width="37.5%"}
    :::

    Eventually, it will reach the base case of $T(1) = 1$.
    The fully expanded tree has height $n$.
    ...
    $$T(n) = 1 + \cdots + (n-2) + (n-1) + n = \frac{n(n+2)}{2} = \Theta(n^2)$$
...
1.  The non-recursive part, $n^2$, will be the parent node
    and the recursive part, $T(n/4)$ and
    $T(n/2)$, will be the child nodes.

    ::: center
    ![image](Figure/4/ex_recursion4.png){width="37.5%"}
    :::

2.  Expand on $T(n/4)$ and $T(n/2)$, similar
    ...
    $$T(n) = n^2\bigg[1 + \Big[\frac{5}{16}\Big] + \Big[\frac{5}{16}\Big]^2 + \cdots\bigg] = \Theta(n^2)$$
...
$b > 1$ and $f(n)$ be a function of
$$f(n) = n^k\log^p{n}$$ Note that there are various
...
1.  *We know $a = 2$ and $b = 2$, but how do
    we get $k$ and $p$? We can rewrite it in
    the form of $n^k\log^p{n}$:
    $$f(n) = 1 = n^0\log^0{n}$$ You can confirm that both
    equations are identical, thus $k = 0$ and
    $p = 0$.*

2.  *Since $\log_2{2} > k$, then
    $T(n) = \Theta(n^{\log_b{a}})$. Substituting in the
    values for $a$ and $b$, we get:
...
1.  When $S\,.\,top = 0$, there is no elements and is
...
2.  When we call [`push(S,15)`]{style="background-color: light-gray"},
    $S\,.\,top$ moves up by $1$ and inserts
    element $15$ to the stack.
...
4.  When we call [`pop(S)`]{style="background-color: light-gray"},
    $S\,.\,top$ moves down by $1$ and returns
    the element that was removed, which is element $3$.

    ::: center
    ![image](Figure/4/ex_stack4.png){height="2cm"}
    :::

5.  Although element $3$ still appears in the array, it is
    no longer in the stack. When we call
    [`push(S,9)`]{style="background-color: light-gray"},
    $S\,.\,top$ moves up by $1$ and overwrites
    element $3$ with $9$.
...
stack, it takes a constant amount of time. Let $n$ be the
...
Assume $n = Q\,.\,length$. The pseudocode for enqueue and
...
2.  When we call
    ...
    $15$ is added to the queue then
    $Q\,.\,tail$ moves up by $1$.
...
4.  When we call [`dequeue(Q)`]{style="background-color: light-gray"},
    element $15$ located at the front of queue indicated by
    $Q\,.\,head$, is removed then $Q\,.\,head$
    moves up by $1$ to element $6$.
...
5.  As the final scenario, suppose we filled the array from
    ...
    one more element, $Q\,.\,tail$ will have to move up by
    one where $Q\,.\,head = Q\,.\,tail$.
...
constant amount of time. Let $n$ be the numbers of elements
in the queue.
...
-   If we attempt to enqueue an element from a full queue, the queue
    overflows and so we can only store $n - 1$ elements.
...
> To search a list of $n$ elements, the
> [List-Search]{.smallcaps} procedure takes $\Theta(n)$
...
> $\text{O}(1)$ time, but if we wish to delete an element
> with a given key, $\text{O}(n)$ time is required in the
> worst case.
...
:   A singly and linked list uses the same algorithm for searching.
    Thus, both take $\Theta(n)$ times in the worst-case to
    search through a list of $n$ elements.
...
:   The max-heap property is that for every node $i$ other
    than the root: $$A[\textsc{Parent}(i)] \geq A[i]$$
...
:   The min-heap property is the opposite, which for every node
    ...
    $$A[\textsc{Parent}(i)] \leq A[i]$$ which means that a
...
an array $A$, where the:

-   Root of tree is $A[1]$. The cell at index
    $0$ is not used, thus we start at index
    $1$.
...
Suppose we have an array $A = [8, 10, 5, 12, 14]$, we would
...
2.  Devise a [`Max-Heapify()`]{style="background-color: light-gray"}
    ...
    $i$ in the heap with sub-trees $l$ and
    $r$ rooted at $i$th children, given to be
    heaps.
...
$\text{O}(n\log{n})$ like merge sort, but heapsort has a
space complexity of $\text{O}(1)$, since it sorts in-place,
...
maintaining a set $S$ of elements, each with an associated
...
-   Using a hash table offers a very fast lookup for a value based on
    the key, which should be the $\text{O}(1)$ operation.
...
:   The simple and straightforward way to lookup number is to check all
    ...
    worst-case search time is $\text{O}(n)$.
...
$$\text{String}\ \Rightarrow\ \boxed{\text{Hash Function}}\ \Rightarrow\ \text{Index}$$
...
array, we store element whose key is $k$ in position
$k$ of the array.
...
::: dBox
::: definitionT
**Definition 6.2** (Direct addressing). *Given a key $k$,
we find the element whose key is $k$ by just looking in the
...
-   Inserting or deleting an element in the table, is the same as you
    ...
    $\text{O}(1)$ time as we already know the index (via
    key).

-   Searching an element takes $\text{O}(1)$ times, as we
...
introduction, instead of storing an element with key $k$ in
index $k$, we use a hash function $h$ and
store the element in index $h(k)$.
...
::: dBox
::: definitionT
**Definition 6.3** (Hash function). *A hash function $h$
...
$\text{O}(1)$ when we talk about insertion, deletion, or
searching an element, the main focus is in its ability to maintains the
size constraint.

The problem with DAT is if the universe $U$ of keys is
large, storing a table of size of $|U|$ may be impractical
or impossible. Often, the set of keys $K$ actually stored
is small, compared to $U$.

::: problem
**Problem 6.1**. *Suppose we have a key of $7898$, which in
...
:   Using a DAT table, we would need a huge array, for the key in index
    $7898$ to store the value at $T[7898]$. In
...
:   But, in the case of a hash table, we can process this key via a hash
    function. The hash function $h(7898)$ maps it to an
    index within the hash table $T[0 \dots n - 1]$.

Regarding the size of the hash table $n$ it typically
...
composition of two functions $h(k) = h_2\big(h_1(k)\big)$:
...
:   By adding up the [[ASCII
    ...
    $$h_1("stop") = 115 + 116 + 111 + 112$$ However, this
...
:   A better hash code takes into account the position of each
    ...
    $$h_1("stop") = (115 \times a^0) + (116 \times a^1) + (111 \times a^2) + (112 \times a^3)$$
    where $a$ is a non-zero constantcompared to \"post\",
...
:   The Multiply-Add-Divide method still use
    $\text{mod}\ n$ to get the numbers in the range, but a
    little fancier by spreading the numbers out first:
    $$h_2(k) = [(ak + b)\ \text{mod}\ p]\ \text{mod}\ n$$
    The values $a$ and $b$ are chosen at
    random as positive integers and $p$ is a prime number,
    where $p > n$. With the addition of
    $(ak + b)\ \text{mod}\ p$, it eliminates patterns
    provided by $k\ \text{mod}\ n$.
...
$7898$ from the previous example and a hash table with
$23$ slots:
$$h_2(7898) = 7898\ \text{mod}\ 23 = 9$$ Then the key will
be mapped to index $9$ of the hash table.
...
$e_1$ and $e_2$ represent the values attached
to $k_1$ and $k_2$ respectively.
...
:   When collision occurs, we linearly probe for the next bucket by
    ...
    $$\text{Index} = \big[h(k) + i\big]\ \text{mod}\ n$$
    where $i$ increases by one each iteration, until it
...
:   Similar to the previous one, but instead we increase the index
    ...
    $$\text{Index} = \big[h(k) + i^2\big]\ \text{mod}\ n$$
    where $i$ increases by one each iteration, until it
    finds an empty bucket.

Double Hashing.

:   Using a secondary hash function $h'(k)$, it places the
    colliding item in the first available cell by:
    $$\text{Index} = \big[h(k) + jh'(k)\big]\ \text{mod}\ n$$
    where $j$ increases by one each iteration, until it
    finds an empty bucket. The secondary hash function cannot have zero
    values and is typically written as such:
    $$h'(k) = q -(k\ \text{mod}\ q)$$ where
    $q$ is a prime number, such that $q > n$.
...
-   Siblings: Children of the same parent ($G, H$ both have
    the parent $C$).
...
-   Ancestors: A node that is connected to all lower-level node
    ($A, B, F$ are ancestors of $I, J, K$).

-   Descendants: The connected lower-level nodes ($I$ is a
    descendant of $A, B, F$).

-   Depth of a node: Number of ancestors ($I$ has a depth
    of $3$).
...
-   The keys satisfy the binary-search tree property:
    $u.key \leq v.key \leq w.key$

    -   Node $u$ is a node (any node) in the left sub-tree
        of node $v$.

    -   Node $w$ is a node (any node) in the right sub-tree
        of node $v$.
...
$\text{O}(h)$, where $h$ is the height of the
tree.
...
reached. The running time for both is $\text{O}(h)$.
...
define what it means. Assuming all keys are unique, if $x$
has two children:
...
2.  Since $20$ is not a left child or located in the left
    sub-tree of $19$, go up the tree to the key value of
    $15$.

3.  Likewise, it is not a left child of $15$, so go up the
    tree to the key value of $25$.

4.  The key value of $25$ has $20$ as a left
    child, therefore, the successor of $20$ is
    $25$.
...
Suppose we want to insert an item with key $9$. The while
...
follows. The running time is $\text{O}(h)$.
...
For binary search trees given we have $n$ items, the
minimum height of a binary tree can be $\log{n}$ and the
maximum be $n$. It can be depicted as such:
...
$\log{n}$, as it provides the worst-case running time of
$\text{O}(\log{n})$, thus comes the motivation for the next
topic.
...
$\log{n}$ is guaranteed when implementing dynamic set of
$n$ items.
...
It ensures the $\text{O}(\log{n})$ time complexity at all
...
dynamic-set operations take $\text{O}(\log{n})$ time in the
worst case.
...
value of $7$. These are all the simple paths that can be
...
of the (same number of) $2$ black nodes. Likewise, the same
...
[Maximum]{.smallcaps}, [Predecessor]{.smallcaps}, and
[Successor]{.smallcaps} in $\text{O}(\log{n})$ time.
...
The letters $\alpha$, $\beta$, and
$\gamma$ represent an arbitrary sub-treeall of them have
...
Push $C$'s black onto $A$ and
$D$. Recurse and check for $C$'s uncle if it
exists.
...
double red nodes ($A$ and $B$) are left or
right children.
...
Suppose we want to insert $15$ using the red-black tree
...
both provide dynamic-set operations in $\text{O}(\log{n})$
...
:   Edges do not have a directionundirected edge are unordered pair of
    vertices, such that $(u,v)$ and $(v,u)$
    are the same edge.
...
:   Edges with directiondirected edges are ordered pair of vertices,
    such that $\langle u,v \rangle$ and
    $\langle v,u \rangle$ are two different edges.
...
$( )$ for unordered pairs and angle brackets
$\langle \rangle$ for ordered pairs.
...
-   The degree of a vertex is the number of incident edges of this
    ...
    $\deg(a)$ be the degree of vertex $a$,
    then $$\sum_{a \in V}\deg(a) = 2m$$
...
$n$ be the number of vertices and $m$ the
number of edges, then $$m \leq \frac{n(n - 1)}{2}$$
...
-   A subgraph of a graph $(V,E)$ is a pair
    $(V', E')$ where $V' \subseteq V$ and
    $E' \subseteq E$. Both endpoints of edges in
    $E'$ are in $V'$.
...
two standard ways to represent a graph $G = (V,E)$, as a
collection of:
...
$\textit{Adj}$ of $|V|$ list, one for each
vertex in $V$.

-   For an undirected graph, the adjacency list
    $\textit{Adj}[u]$ contains all the vertices
...
storing it with vertex $v$ in $u$'s adjacency
list.
...
$(u,v)$ is present in the graph.

1.  We would need to search for $v$ in the adjacency list
    $\textit{Adj}[u]$.

2.  If we want to check the edge $(2,4)$, then we would
    need search through $\textit{Adj}[2]$.
...
requires a constant time $\text{O}(1)$, but at the cost of
using asymptotically more memory.

-3ex -0.1ex -.4ex 0.5ex .2ex Adjacency Matrix The adjacency-matrix
representation of a graph consists of a $|V| \times |V|$
matrixassuming the vertices are numbered from $1$,
$2$, ..., to $|V|$.

We can represent the elements inside the matrix $A$ as
$a_{ij}$, where $i$ and $j$
indicate the row and column.

-   For an undirected graph, if there is an edge
    $(i,j), (j,i) \in E$, then set
    ...
    Notice how $a_{22}$ is $0$, since the edge
    $(2,2)$ does not exist.
...
-   For a directed graph, if there is an edge
    $\langle i, j \rangle \in E$, then set
    $a_{ij} = 1$, otherwise, $a_{ij} = 0$.
...
can represent a weighted graph. Instead of storing $0$'s
and $1$'s, we store the weight of the given edge.
...
advantages and disadvantages.
   ...
   **Time:** List all vertices adjacent to $u$   $\Theta(\deg(u))$    $\Theta(|V|)$
      **Time:** Determine if $(u,v) \in E$       $\Theta(\deg(u))$     $\Theta(1)$
...
-   The adjacency-list representation provides a compact way to
    represent sparse graphsthose for which $|E|$ is much
    less than $|V|^2$.

-   However, if $|E|$ is close to $|V|^2$,
...
1.  Let's start at the root of tree. Let's add $A$ to the
    queue.
...
2.  We want to explore all the vertices that are adjacent to
    ...
    could have stored $C$ first.

3.  Since we finished \"exploring\" $A$, we will move on,
    then $B$ is next in queue.

    ::: center
    ![image](Figure/8/BFS/tree3.png){height="3cm"}
    :::

4.  Likewise, we add all vertices adjacent to $B$, which
    are $D$ and $E$, to the queue.

    ::: center
    ![image](Figure/8/BFS/tree4.png){height="3cm"}
    :::

5.  Since we finished \"exploring\" $B$ , we will move to
    $C$.
...
6.  We will add the vertices adjacent to $C$. which are
    $F$ and $G$ to the queue.

    ::: center
    ![image](Figure/8/BFS/tree6.png){height="3cm"}
    :::

7.  When we move to vertex $D$, you will see there are no
...
8.  The same can be said for $E$ through $G$,
...
2.  Then we arbitrarily pick an edge outwards of $A$, which
    will choose $B$ and add to the stack.
...
6.  Then we backtrack all the way to vertex $A$, since all
    of vertex $B$ and $E$ has been explored.
...
$\text{O}(V + E)$, similar to [BFS]{.smallcaps} procedure.
...
$\text{O}(V + E)$ time, since DFS takes
$\text{O}(V + E)$.
...
1.  We start off by selecting the smallest edge in the graph, which is $(6,1)$, as it has an edge weight of $10$.

    ::: center
    ![image](Figure/8/MST/k1.png){height="3cm"}
    :::

2.  Then, we select the next smallest edge, which is $(4,3)$, with an edge weight of $12$.

    ::: center
    ![image](Figure/8/MST/k2.png){height="3cm"}
    :::

3.  The next minimum edge is $(7,2)$, with an edge weight of $14$.

    ::: center
    ![image](Figure/8/MST/k3.png){height="3cm"}
    :::

4.  The next minimum edge is $(2,3)$, with an edge weight of $16$.

    ::: center
    ![image](Figure/8/MST/k4.png){height="3cm"}
    :::

5.  The next minimum edge is $(7,4)$, with an edge weight of $18$. However, notice that it forms a cycle, so we discard it.
...
6.  The next minimum edge is $(5,6)$, with an edge weight of $25$.

    ::: center
    ![image](Figure/8/MST/k7.png){height="3cm"}
    :::

7.  Once we have all our vertices or have $|V| - 1$ edges, we are done with the algorithm. Our MST is complete, which has the weight of $99$.
...
1.  Since we started with $6$, we have to select the smallest edge that is connected to $6$, which are either: $25$ or $10$.
...
2.  Likewise, we now have to select the smallest edge that is connected to either $6$ or $1$, which will pick edge $(6,5)$, with an edge weight of $25$.

    ::: center
    ![image](Figure/8/MST/p2.png){height="3cm"}
    :::

3.  We are basically repeating the same step, but now with more and more edges to pick from. The dashed lines indicate the possible edges to select.
...
4.  Once we have all our vertices or have $|V| - 1$ edges, we are done with the algorithm, which also have a weight of $99$.
...
1.  Let's pick $A$ as our source vertex. There's a table in the right-side, which will use to keep track of distances from the source vertex to each vertex. 
...
2.  Next, we examine the edges leaving $A$. As denoted in  the table, we can reach $B$ and $C$ with an edge weight of $10$ and $3$ respectively.

    ::: center
    ![image](Figure/8/SP/djk2.png){height="3cm"}
    :::

3.  We always pick the smallest edge weight of which the vertex hasn't been discovered. In this case, it's $\langle A,C \rangle$. 
...
4.  Now $D$ and $E$ are now reachable. As usual, we'll pick the next smallest edge, which is $\langle C, E \rangle$. 
...
5.  Our only options left are $\langle C,B \rangle$ and $\langle C,D \rangle$. We pick $\langle C, B \rangle$, as it has the smallest edge weight of $7$.
...
6.  So all that's left is to pick the last remaining edge, which is $\langle B, D \rangle$.

    ::: center
    ![image](Figure/8/SP/djk6.png){height="3cm"}
    :::

7.  All the edges have been discovered, so we are done with the algorithm.
...
1.  As usual, let start with vertex $A$ as our source vertex and the distances to each vertex listed in the right.

    ::: center
    ![image](Figure/8/SP/djk1.png){height="3cm"}
    :::

2.  With Bellman-Ford's algorithm, we want to relax all the edges. In other words, we should test out all the possible edges that will result in the shortest path.

3.  So this will be our first iteration. Starting at $A$, we can reach $B$ and $C$ with a weight of $10$ and $3$, using the edges, $\langle A,B \rangle$ and $\langle A,C \rangle$ respectively.

    ::: center
    ![image](Figure/8/SP/bmf1.png){height="3cm"}
    :::

4.  From $B$, we can reach $D$ at a total weight of $12$ using the edge  $\langle B,D \rangle$. Note that we won't use the edge $\langle B,C \rangle$, as it result in a longer path from $A$ to $C$.

    ::: center
    ![image](Figure/8/SP/bmf2.png){height="3cm"}
    :::

5.  From $C$, we can reach $E$ at a total weight of $5$ using the edge $\langle C,E \rangle$. 
...
6.  From $D$, we only have one edge to work with, which is $\langle D,E \rangle$, however, notice how this increases the total weight of $E$ from $5$ to $16$, so, we don't use it.

    ::: center
    ![image](Figure/8/SP/bmf5.png){height="3cm"}
    :::

7.  Similarly for $E$, we have the edge $\langle E,D \rangle$, however, this also increase the weight of $D$ by $9$ to $12$, so we don't use it.
...
8.  The algorithm at most takes $|V| - 1$ iterations to fully obtain the shortest-tree path. However, it can sometimes be less if there's no changes occurring after the next iteration. 

9.  In our second iteration, we proceed to the do the following, but using the graph we got from our first iteration.
...
10. Same thing can be said for $B$, $C$, $D$ and $E$. So after our second iteration, we are finished.
